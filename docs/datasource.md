...
Snapshots are copied and recursively frozen at runtime; all nested properties and collections are deeply readonly in the public interface. Version 0.4 intentionally rejects writes that already failed at runtime in 0.3. Use `simulatorDatasourceToPayload` to obtain a mutable session copy. Typed hosts with already validated session data can use `createSimulatorDatasourceFromPayload`; this helper only performs the existing minimal session validation and is not an untrusted-input validator.
...
Keep datasource identity stable until content changes. Convert it once with `simulatorDatasourceToPayload` (for example, in `useMemo` keyed by datasource), then use `updateSimulatorPayload(state, payload)` to reconcile navigation against that mutable session copy. The convenience `updateSimulatorDatasource` still makes a fresh copy on each call. Supply either a precomposed `state` or a `datasource` overlay to `SimulatorPhoneDevice`; do not convert the same content in both the host and device. Replacement preserves the existing view state and mounted component drafts. A selected email removed from all folders is reconciled to its list. Other missing records continue to use existing view fallbacks. The legacy `value` replacement behavior still starts a new session. Hosts replacing the scenario itself should explicitly remount/reset the session; snapshot refresh is intended for updates within the same scenario. Reducer actions and choices remain session-owned and callbacks remain separate from content.
